...
{{
  ...
  "name": "<שם שהמשתמש הזכיר (לדוגמה אם ענה רק בשם שלו, יש לחלץ אותו לכאן), או null>",
...
חוקי סיווג intent:
- APPOINTMENT_INQUIRY  : המשתמש מבקש מידע על תור אישי שלו, או ביטול תור.
...
- IDENTITY_RESPONSE    : המשתמש עונה על שאלת זיהוי - מספק שם (גם אם מילה אחת כמו "ישראל"), או ת.ז.
...
חוקי פורמט (קריטי):
- name: אם המשתמש מקליד מילה בודדת שיכולה להיות שם פרטי (למשל: "ישראל", "דוד", "רונן"), חובה לחלץ אותה לשדה name ולסווג כ-IDENTITY_RESPONSE או CLARIFICATION!
